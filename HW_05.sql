...
CREATE VIEW `cars_after_25k` AS
SELECT * 
FROM `cars`
WHERE `cost` <= 25000;

SELECT * FROM `cars_after_25k`;

-- 2. Изменить в существующем представлении порог для стоимости: пусть цена будет до 30 000 долларов (используя оператор ALTER VIEW)
ALTER VIEW `cars_after_25k` AS
SELECT * 
FROM `cars`
WHERE `cost` <= 30000;
...
SELECT *,
TIMEDIFF(LEAD(`station_time`) OVER (PARTITION BY `train_id`), `station_time`) AS `station_to_next`
FROM `stations`;
